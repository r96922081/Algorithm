...
void list_insert_after_2(struct list* l, struct list_node* n, struct list_node* new_n)
{
    if (n == l->tail)
    {
        l->tail->next = new_n;
...
void list_insert_before_2(struct list* l, struct list_node* n, struct list_node* new_n)
{
    if (n == l->head)
    {
        l->head->prev = new_n;
...
list_node* list_insert_before(struct list* l, struct list_node* n, void* v)
{
    ...
    list_insert_before_2(l, n, new_n);
...
void list_clear(struct list* l)
{
    while (l->head != NULL)
    {
        struct list_node* prev_head = l->head;
        l->remove(l, l->head);
        my_free(prev_head);
    }
}

void list_clear_and_free_pointer(struct list* l)
{
...
list* new_list()
{
    ...
    l->insert_after_2 = list_insert_after_2;
    l->insert_before_2 = list_insert_before_2;
    l->clear = list_clear;
    l->clear_and_free_pointer = list_clear_and_free_pointer;
    l->print = list_print;
    return l;
}
